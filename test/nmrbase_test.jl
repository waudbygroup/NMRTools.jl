...
@testset "NMRBase: list size mismatch handling" begin
    using NMRTools.NMRIO: _find_outermost_dim_index, _adjust_list_size

    # --- _find_outermost_dim_index tests ---

    # Missing acquisition_order: falls back to ndims
    ann_no_order = Dict{String,Any}("dimensions" => ["f1", "relaxation.duration"])
    @test _find_outermost_dim_index(ann_no_order, 2) == 2

    # Integer acquisition_order: [innermost..., outermost] where values are
    # 1-based indices into the ORIGINAL (pre-reversal) dimensions array.
    # Original dims: ["relaxation.duration", "spinlock_power", "f1"] (indices 1,2,3)
    # acquisition_order [3,1,2]: outermost = 2 (spinlock_power)
    # Reversed dim index = ndims+1-2 = 3+1-2 = 2
    ann_int = Dict{String,Any}("dimensions" => ["f1", "spinlock_power", "relaxation.duration"],
                               "acquisition_order" => [3, 1, 2])
    @test _find_outermost_dim_index(ann_int, 3) == 2

    # String acquisition_order: last entry = outermost name in ORIGINAL array
    # Original dims (pre-reversal): ["cest.offset", "f1"]
    # acquisition_order ["f1", "cest.offset"]: outermost = "cest.offset" at original index 1
    # Reversed dim index = 2+1-1 = 2
    ann_str = Dict{String,Any}("dimensions" => ["f1", "cest.offset"],
                               "acquisition_order" => ["f1", "cest.offset"])
    @test _find_outermost_dim_index(ann_str, 2) == 2

    # --- _adjust_list_size tests ---

    spec = exampledata("pseudo2D_XSTE")  # 2048 x 10
    ndata = size(spec, 1)  # 10 (pseudo dimension is dim 1 in this dataset)

    # Find which dim is the pseudo dim (size 10)
    outer_dim = findfirst(d -> size(spec, d) == 10, 1:ndims(spec))

    # List longer than data: truncate list
    long_values = collect(1.0:15.0)  # 15 entries, data has 10
    @test_logs (:warn,) begin
        _, adjusted = _adjust_list_size(spec, outer_dim, long_values, true)
        @test length(adjusted) == 10
        @test adjusted == 1.0:10.0
    end

    # List shorter than data: extend cyclically
    short_values = [0.01, 0.02, 0.04]  # 3 entries, data has 10
    @test_logs (:warn,) begin
        _, adjusted = _adjust_list_size(spec, outer_dim, short_values, true)
        @test length(adjusted) == 10
        @test adjusted[1] ≈ 0.01
        @test adjusted[2] ≈ 0.02
        @test adjusted[3] ≈ 0.04
        @test adjusted[4] ≈ 0.01  # wraps back to start
        @test adjusted[end] ≈ short_values[mod(9, 3) + 1]
    end

    # Exact match: no adjustment, no warning
    exact_values = collect(1.0:10.0)
    spec2, adjusted2 = _adjust_list_size(spec, outer_dim, exact_values, true)
    @test adjusted2 === exact_values
    @test spec2 === spec

    # Inner loop mismatch: throw error
    @test_throws NMRToolsError _adjust_list_size(spec, outer_dim, long_values, false)
end
