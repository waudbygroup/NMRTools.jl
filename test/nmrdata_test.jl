...
@testset "Creating axes" begin
    ax1 = X(x1, UnknownGrid(), Dict())
    ax2 = Y(x2, UnknownGrid(), Dict())
    ax3 = Z(x1, UnknownGrid(), Dict())
    ax4 = Ti(x2, UnknownGrid(), Dict())
    @test ax1[1] == 0.0
    @test val(ax1) == x1
...
@testset "Creating NMRData indirectly via DimensionalData" begin
    A = DimensionalArray(y, (X(x1, UnknownGrid(), Dict()), Y(x2, UnknownGrid(), Dict())));
    @test A[1,1] == 2
    d = NMRData(A, md)
    @test d[1,1] == 2
end
...
@testset "Creating NMRData directly" begin
    d = NMRData(y, (X(x1, UnknownGrid(), Dict()), Y(x2, UnknownGrid(), Dict())), md)
    @test d[1,1] == 2
    @test d == y
