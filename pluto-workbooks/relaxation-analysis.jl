...
md"# NMRTools relaxation analysis"

# ╔═╡ 35a2b544-ff30-4b52-a82d-401fc6f73f8f
begin
	# select input data
	inputfile = "/Users/chris/.julia/dev/NMRTools/exampledata/pseudo2D_XSTE/1"
	
	# specify relaxation times
	tau = [0.0, 1., 2, 4, 7, 10, 20, 40, 70, 100] * 1e-3 # convert ms to s
	
	# select chemical shift ranges for plotting and fitting
	plotrange = 6 .. 10
	selector = 7.8 .. 7.9

	# pick an empty point in the spectrum to estimate the noise
	noise_position = 10.5 # ppm
...
begin
	...
	spec = setrelaxtimes(dat, tau, "s")

	noisewidth = selector.right - selector.left
	noiseselector = (noise_position-0.5noisewidth)..(noise_position+0.5noisewidth)

	noiseintegrals = vec(data(sum(spec[noiseselector,:],dims=F1Dim)))
	noise = std(noiseintegrals)
    ...
    p1 = scatter(tau, integrals .± noise, label="observed",
			frame=:box,
...
begin
    ...
    DownloadButton(take!(buf), "relaxation-fit.pdf")
end

# ╔═╡ 16a9efa6-bd1a-46f2-9403-06f916fc4546
begin
	println("tau/s\tintegral\tnoise\tfit")
	for i=1:length(tau)
		println("$(tau[i])\t$(integrals[i])\t$noise\t$(model(tau[i],pfit))")
	end
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "1ac9feeca3253ea4e6065fcb3ab95fdbac4ef754"
...
git-tree-sha1 = "7498398024268efa2c18a8d34ff532868ae01551"
...
git-tree-sha1 = "c2d806b937bbcd057b0384dc6dd05d37c5df7770"
