...
function correlationlength(w::ExponentialWindow, sw, n)
    # Correlation length = linewidth in points = lb / (Hz per point) = lb * n / sw
    w.lb > 0 ? w.lb * n / sw : 1.0
end

# TODO: derive proper correlation length for sine windows
correlationlength(w::CosWindow, sw, n) = 2.0
correlationlength(w::Cos²Window, sw, n) = 2.0
correlationlength(w::GeneralSineWindow, sw, n) = 2.0

# Gaussian windows: dominated by Gaussian broadening
function correlationlength(w::LorentzToGaussWindow, sw, n)
    w.gaussHz > 0 ? w.gaussHz * n / sw : 4.0
end

function correlationlength(w::GeneralGaussWindow, sw, n)
    w.gaussHz > 0 ? w.gaussHz * n / sw : 4.0
end
